...
const navItems = [
  ...
  { href: "/smart-run", label: "Smart Run", icon: "Bot" },
  { href: "/run", label: "Run Tests", icon: "Play" },
  { href: "/record", label: "Record Tests", icon: "Zap" },
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 h-screen w-64 border-r border-border bg-card">
      <div className="flex h-14 items-center border-b border-border px-4">
        <span className="text-lg font-bold">🎭 Shopify Autotest</span>
      </div>
      <nav className="space-y-1 p-3">
        {navItems.map((item) => (
          ...
          >
            {icons[item.icon]}
            {item.label}
            {pipelineRunning && item.href === "/smart-run" && (
              <span className="ml-auto h-2 w-2 rounded-full bg-amber-400 animate-pulse" title="Pipeline running" />
            )}
          </Link>
        ))}
      </nav>
